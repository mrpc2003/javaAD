...
class Game  								//전체 게임 관할
{
	String Bp1name;
	String Bp2name;
	Player p1 = new Player(Bp1name);			// 이름과 함께 p1 지정 (메인 게임에서의 이름 받아오도록)
	Player p2 = new Player(Bp2name);			// 이름과 함께 p2 지정
	Dealer dealer = new Dealer();			// 딜러 생성
	CardDeck carddeck = new CardDeck();		// 전체 카드 덱 생성
	int playCount = 0;						// 몇 라운드 돌지 정하는 변수
