...
public class Main {
	...
	private static int[] rollSelectedDice(int[] dice, int[] diceIdx) { // 선택한 주사위를 던지는 메소드
		for (int i = 0; i < diceIdx.length; i++) { // 선택한 주사위의 개수만큼 반복
			dice[diceIdx[i] - 1] = genRandom(); // 선택한 주사위를 던짐
		}
		return dice; // 던진 주사위를 반환
	}
	...
	private static void p1inputScore(int[] dice, ScoreBoard sb) throws IOException { // 플레이어 1이 점수를 넣을 항목을 입력하는 메소드
		...
		String element; // 입력받은 점수를 저장하기 위한 변수
		...
		while (inputLoop) {// 여기부터 모든 항목에 대한 분기점을 넣어야하기 때문에 굉장히 길어짐, 노가다 작업 Start
			...
			element = br.readLine(); // 점수를 넣을 항목을 입력받음
			temp=""; // temp를 초기화
			StringTokenizer st = new StringTokenizer(element); // 입력받은 점수를 공백을 기준으로 분리
			element=""; // element를 초기화
			...
			for (int i = 0; i < temp.length(); i++) {// 전부 소문자로 변환하는 작업
				if (Character.isLetter(temp.charAt(i)) && temp.charAt(i) >= 'A' && temp.charAt(i) <= 'Z') {// 문자&A~Z사이
																											// 체크
					element += (char) (temp.charAt(i) + 32); // 대문자를 소문자로 변환
			...
			} else if (element.equals("deuces")) { // 위와 동일
			...
			} else if (element.equals("threes")) { // 위와 동일
			...
			} else if (element.equals("fours")) { // 위와 동일
			...
			} else if (element.equals("fives")) { // 위와 동일
			...
			} else if (element.equals("sixes")) { // 위와 동일
			...
			} else if (element.equals("choice")) { // 위와 동일
			...
			} else if (element.equals("4ofakind")) { // 위와 동일
			...
			} else if (element.equals("fullhouse")) { // 위와 동일
			...
			} else if (element.equals("smallstraight")) { // 위와 동일
			...
			} else if (element.equals("largestraight")) { // 위와 동일
			...
			} else if (element.equals("yacht")) { // 위와 동일
